...
void setup(){
  ...
  DDRH |= B00001000;
  DDRG |= B00100000;
  DDRB |= B00000001;                                                                    //Configure digital poort 12 as output.
  ...
  while(eeprom_data[33] != 'J' || eeprom_data[34] != 'M' || eeprom_data[35] != 'B'){
    ...
    digitalWrite(53, !digitalRead(53));                                                 //Change the led status to indicate error.
  }
...
void loop(){
  ...
  if(receiver_input_channel_3 < 1100)new_function_request = false;                      //If the throttle is in the lowest position set the request flag to false.
  ...
  if(data == 'a'){
    ...
    if(cal_int != 2000){
      ...
      for (cal_int = 0; cal_int < 2000 ; cal_int ++){                                   //Take 2000 readings for calibration.
        if(cal_int % 125 == 0){
          digitalWrite(53, !digitalRead(53));   //Change the led status to indicate calibration.
          Serial.print(".");
        }
...
void esc_pulse_output(){
  zero_timer = micros();
  PORTE |= B00101000;                                            //Set port 4, 5, 6 and 7 high at once
  PORTH |= B00001000;
  PORTG |= B00100000;
