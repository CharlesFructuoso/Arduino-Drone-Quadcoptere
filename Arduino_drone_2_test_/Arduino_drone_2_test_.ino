...
double pitch_ki=0.001;//0.003
double pitch_kd=0.25;//2.05
...
float yaw_pid_d=0;
///////////////////////////////YAW PID CONSTANTS///////////////////
double yaw_kp=1;//3.55
double yaw_ki=0.001;//0.003
double yaw_kd=0.25;//2.05
...
//  PORTB &= B11011111; //D13 set to LOW
  ...
  R_F_prop.attach(3); //right front motor 
 ...
 * I make the mean of 200 values, that should be enough*/
  if(acc_error==0)
  {
    for(int a=0; a<200; a++)
    {
      ...
      /*---Z---*/
      Acc_angle_error_z = Acc_angle_error_z + ((atan((Acc_rawX)/sqrt(pow((Acc_rawZ),2) + pow((Acc_rawY),2)))*rad_to_deg));      
      if(a==199)
      {
...
//  }
  ...
  noInterrupts();
  unsigned long input_THROTTLE = pulseIn(10, HIGH);
  unsigned long Yaw = pulseIn(12, HIGH);
  unsigned long Pitch = pulseIn(9, HIGH);
  unsigned long Roll = pulseIn(8, HIGH);
  interrupts();
...
roll_desired_angle = map(Roll,1000,2000,-10,10);
pitch_desired_angle = map(Pitch,1000,2000,-10,10);
yaw_desired_angle = map(Yaw,1000,2000,-10,10);

Serial.println(roll_desired_angle);
