...
#define ECHO 38    // Broche ECHO
#define ECHO1 40    // Broche ECHO
#define ECHO2 48    // Broche ECHO
                                               // definition du Timeout
Metro sensor1 = Metro(125);
Metro sensor= Metro(150);
Metro sensor2= Metro(175);
Metro gps_delay = Metro(200);
...
float pid_p_gain_yaw = 0.5;                //Gain setting for the pitch P-controller.
...
void setup(){
  ...
  DDRE |= B00101000;                                                        //Configure digital poort 4, 5, 6 and 7 as output.
  DDRH |= B00001000;
  DDRG |= B00100000;
  DDRB |= B00000011;                                                        //Configure digital poort 12 and 13 as output.

  //Use the led on the Arduino for startup indication.
  digitalWrite(53,HIGH);                                                    //Turn on the warning led.
  ...
  for (cal_int = 0; cal_int < 1250 ; cal_int ++){                           //Wait 5 seconds before continuing.
    PORTE |= B00101000;                                                     //Set digital poort 4, 5, 6 and 7 high.
    PORTH |= B00001000;
    PORTG |= B00100000;
    delayMicroseconds(1000);                                                //Wait 1000us.
    PORTE &= B11010111;                                                     //Set digital poort 4, 5, 6 and 7 low.
    PORTH &= B11110111;
    PORTG &= B11011111;
  ...
  for (cal_int = 0; cal_int < 2000 ; cal_int ++){                           //Take 2000 readings for calibration.
    if(cal_int % 15 == 0)digitalWrite(53, !digitalRead(53));                //Change the led status to indicate calibration.
  ...
  while(receiver_input_channel_3 < 990 || receiver_input_channel_3 > 1020 || receiver_input_channel_4 < 1400){
    ...
    PORTE |= B00101000;
    PORTH |= B00001000;
    PORTG |= B00100000;                                                     //Set digital poort 4, 5, 6 and 7 high.
    delayMicroseconds(1000);                                                //Wait 1000us.
    PORTE &= B11010111;
    PORTH &= B11110111;
    PORTG &= B11011111;                                                     //Set digital poort 4, 5, 6 and 7 low.
    delay(3);                                                               //Wait 3 milliseconds before the next loop.
    if(start == 125){                                                       //Every 125 loops (500ms).
      digitalWrite(53, !digitalRead(53));                                   //Change the led status.
  ...
  digitalWrite(53,LOW);                                                     //Turn off the warning led.
}
...
//     closeCommand+="\r\n";
  ...
  if(receiver_input_channel_3 < 1050 && receiver_input_channel_4 < 1100)start = 1;
  //When yaw stick is back in the center position start the motors (step 2).
  if(start == 1 && receiver_input_channel_3 < 1050 && receiver_input_channel_4 > 1400){
    start = 2;
  ...
  digitalWrite(TRIGG, LOW);
    if(sensor.check()){
  mesure = pulseIn(ECHO, HIGH, TIMEOUT); // Mesure le temps entre 
                                          // l'envoi de l'ultrason et sa réception
    }
    if(sensor1.check()){
  mesure1 = pulseIn(ECHO1, HIGH, TIMEOUT); // Mesure le temps entre 
                                          // l'envoi de l'ultrason et sa réception
    }
    if(sensor2.check()){
  mesure2 = pulseIn(ECHO2, HIGH, TIMEOUT); // Mesure le temps entre 
                                          // l'envoi de l'ultrason et sa réception
    }
  float distance_mm1 = mesure1 / 2.0 * son; //calcul de la distance grâce au temps
                                      //on divise par 2 car le son fait un aller-retour
  float distance_mm = mesure / 2.0 * son; //calcul de la distance grâce au temps
                                      //on divise par 2 car le son fait un aller-retour
  float distance_mm2 = mesure2 / 2.0 * son; //calcul de la distance grâce au temps
                                      //on divise par 2 car le son fait un aller-retour
  ...
  if(distance_mm1 > 200 && distance_mm1 < 600)pid_roll_setpoint=200; 
  else if(distance_mm2 < 600 && distance_mm2 > 200)pid_roll_setpoint = -200;
  ...
  if(distance_mm > 200 && distance_mm < 600)pid_pitch_setpoint=200;  
  ...
  //if(battery_voltage < 1000 && battery_voltage > 600)digitalWrite(53, HIGH);
...
//    } 
  ...
  if(micros() - loop_timer > 4050)digitalWrite(53, HIGH);                   //Turn on the LED if the loop time exceeds 4050us.
//  
...
//  //The refresh rate is 250Hz. That means the esc's need there pulse every 4ms.
  ...
  PORTE |= B00101000;                                                       //Set digital outputs 4,5,6 and 7 high.
  PORTH |= B00001000;
  PORTG |= B00100000;
  ...
  while(PORTE >= 8 || PORTG >= 32 || PORTH >= 8){                                                       //Stay in this loop until output 4,5,6 and 7 are low.
    esc_loop_timer = micros();                                              //Read the current time.
    if(timer_channel_4 <= esc_loop_timer)PORTE &= B11110111;                //Set digital output 5 to low if the time is expired.
    if(timer_channel_3 <= esc_loop_timer)PORTE &= B11011111;                //Set digital output 3 to low if the time is expired.
    if(timer_channel_1 <= esc_loop_timer)PORTG &= B11011111;                //Set digital output 4 to low if the time is expired.
    if(timer_channel_2 <= esc_loop_timer)PORTH &= B11110111;                //Set digital output 6 to low if the time is expired.
  }
