...
int input_YAW;      //In my case channel 4 of the receiver and pin D13 of arduino
int input_PITCH;    //In my case channel 2 of the receiver and pin D11 of arduino
int input_ROLL;     //In my case channel 1 of the receiver and pin D10 of arduino
...
float pitch_pid_d=0;
///////////////////////////////PITCH PID CONSTANTS///////////////////
double pitch_kp=0.7;//3.55
double pitch_ki=0;//0.003
double pitch_kd=0.04;//2.05
...
void setup() {
  ...
  PCMSK0 |= (1 << PCINT5);  //Set pin D11 trigger an interrupt on state change.                                             
  PCMSK0 |= (1 << PCINT6);  //Set pin D12 trigger an interrupt on state change.                                               
  PCMSK0 |= (1 << PCINT7);  //Set pin D13 trigger an interrupt on state change.  

  
  DDRB |= B00000010;  //D13 as output
  PORTB &= B11111101; //D13 set to LOW

  L_F_prop.attach(5); //left front motor
  L_B_prop.attach(3); //left back motor
  R_F_prop.attach(4); //right front motor 
...
*/
  ...
  if(input_THROTTLE < 1200 && input_YAW > 1700 && !mot_activated)
  {
    if(activate_count==200)
    {
      mot_activated=1;   
      PORTB |= B00000010; //D13 LOW   
  ...
  if(!(input_THROTTLE < 1200 && input_YAW > 1700) && !mot_activated)
  {
   ...
   if(input_THROTTLE < 1200 && input_YAW < 1200 && mot_activated)
  {
    if(des_activate_count==300)
    {
      mot_activated=0;       
      PORTB &= B11111101; //D13 LOW   
    }
...
//First we take the current count value in micro seconds using the micros() function
  ...
  ///////////////////////////////////////Channel 2
  if(PINB & B00100000 ){                             //pin D9 -- B00000010                                              
    if(last_CH2_state == 0){                                               
      last_CH2_state = 1;                                                   
  ...
  ///////////////////////////////////////Channel 3
  if(PINB & B01000000 ){                             //pin D10 - B00000100                                         
    if(last_CH3_state == 0){                                             
      last_CH3_state = 1;                                                  
  ...
  ///////////////////////////////////////Channel 4
  if(PINB & B10000000 ){                             //pin D12  -- B00010000                      
    if(last_CH4_state == 0){                                               
      last_CH4_state = 1;                                                   
